-- Phase 1: Création de données de test complètes pour CIARA (corrigé avec les bons enums pour steps)
...
-- Création des catégories de parcours pour chaque ville
...
-- Création des étapes (steps) pour chaque ville avec les bons types step_type
...
((SELECT id FROM cities WHERE slug = 'geneve'), 'Jardin Botanique', 'Oasis de verdure avec 16 000 espèces', 'Chemin de l''Impératrice 1, 1292 Chambésy', 46.2267, 6.1433, 'viewpoint', 20, 45, true, ARRAY['https://images.unsplash.com/photo-1465146344425-f00d5f5c8f07']),
...
((SELECT id FROM cities WHERE slug = 'zurich'), 'Lac de Zurich', 'Promenade au bord du lac avec vue sur les Alpes', 'Seepromenade, 8001 Zürich', 47.3656, 8.5489, 'viewpoint', 15, 40, true, ARRAY['https://images.unsplash.com/photo-1506744038136-46273834b3fb']),
...
((SELECT id FROM cities WHERE slug = 'berne'), 'Fosse aux ours', 'Parc des ours emblématiques de Berne', 'Grosser Muristalden 6, 3006 Bern', 46.9467, 7.4558, 'viewpoint', 20, 40, true, ARRAY['https://images.unsplash.com/photo-1452378174528-3090a4bba7b2']),
...
((SELECT id FROM cities WHERE slug = 'montreux'), 'Vignobles de Lavaux', 'Terrasses de vignes UNESCO', 'Route de la Corniche, 1071 Chexbres', 46.4789, 6.8333, 'viewpoint', 20, 60, true, ARRAY['https://images.unsplash.com/photo-1509316975850-ff9c5deb0cd9']),
...
((SELECT id FROM cities WHERE slug = 'interlaken'), 'Lac de Thoune', 'Lac alpin avec vue sur l''Eiger', 'Seestrasse, 3800 Interlaken', 46.6889, 7.8581, 'viewpoint', 15, 40, true, ARRAY['https://images.unsplash.com/photo-1500673922987-e212871fec22']),
