...
SELECT 
  ...
  CASE 
    WHEN s.type = 'monument' THEN '["Il témoigne de l''histoire locale", "Il a été construit récemment", "Il n''a pas d''importance", "Il va être démoli"]'::jsonb
    WHEN s.type = 'viewpoint' THEN '["Un panorama exceptionnel", "Seulement des bâtiments", "Rien d''intéressant", "Uniquement la mer"]'::jsonb
    WHEN s.type = 'museum' THEN '["Des collections d''art et d''histoire", "Seulement des voitures", "Uniquement des vêtements", "Rien d''important"]'::jsonb
    ELSE '["C''est un lieu important de la ville", "C''est un lieu sans intérêt", "Il n''existe plus", "C''est privé"]'::jsonb
  END as options,
  CASE 
