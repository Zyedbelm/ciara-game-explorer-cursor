-- Add demo data for Sion (corrected types)
-- Journey categories for Sion
...
-- Rewards for Sion partners (using proper enum types)
...
SELECT 
  ...
  END,
  CASE 
    WHEN p.category = 'restaurant' THEN 'discount'::reward_type
    WHEN p.category = 'wine' THEN 'free_item'::reward_type
    WHEN p.category = 'hotel' THEN 'upgrade'::reward_type
  END,
  CASE 
