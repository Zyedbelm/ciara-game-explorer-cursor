-- Phase 1: Création de données de test complètes pour CIARA
...
-- Lausanne
((SELECT id FROM cities WHERE slug = 'lausanne'), 'Patrimoine UNESCO', 'patrimoine-unesco-lausanne', 'heritage', 'Découvrez le patrimoine architectural et culturel de Lausanne', 'easy', 90, 'building', '#8B5CF6'),
...
((SELECT id FROM cities WHERE slug = 'lausanne'), 'Street Art urbain', 'street-art-lausanne', 'art', 'Circuit artistique dans les quartiers branchés', 'easy', 120, 'palette', '#F59E0B'),

-- Genève
((SELECT id FROM cities WHERE slug = 'geneve'), 'Genève International', 'geneve-international', 'heritage', 'Sur les traces des organisations internationales', 'medium', 150, 'globe', '#E74C3C'),
...
-- Zurich
((SELECT id FROM cities WHERE slug = 'zurich'), 'Vieille ville de Zurich', 'vieille-ville-zurich', 'heritage', 'Cœur historique entre églises et guildes', 'easy', 105, 'church', '#2ECC71'),
((SELECT id FROM cities WHERE slug = 'zurich'), 'Zurich moderne', 'zurich-moderne', 'urban', 'Architecture contemporaine et quartiers branchés', 'medium', 135, 'building', '#E67E22'),
...
-- Berne
((SELECT id FROM cities WHERE slug = 'berne'), 'Centre historique', 'centre-historique-berne', 'heritage', 'Arcades médiévales et fontaines historiques', 'easy', 120, 'castle', '#9B59B6'),
...
-- Montreux
((SELECT id FROM cities WHERE slug = 'montreux'), 'Riviera et châteaux', 'riviera-chateaux', 'heritage', 'Château de Chillon et promenade lakeside', 'medium', 180, 'castle', '#1ABC9C'),
